...
do
  ...
  python -u train.py +model=SCBM +data=$data \
    model.cov_type='amortized' model.reg_precision='l1' model.reg_weight=1 \
    experiment_name="${data}_SCBM_amortized_${i}" seed=$i \
    logging.project=SCBM logging.mode=offline model.tag=$tag \
